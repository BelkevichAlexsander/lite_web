...
class TestViews(MyTestSetting):
    ...
    def test_POST(self):
        response = self.client.post(reverse('lite:version'), data={
            'software': "software blank POST",
            'version': 'version blank POST'
        })

        self.client.post(reverse('lite:version'), data={
            'software': "soft",
            'version': 'vers'
        })
    ...
    def test_PUT(self):
        response = self.client.put(reverse('lite:version/pk', kwargs={'pk': 0}),
    ...
    #         }
    #     ]
    #     response = self.client.delete(reverse('lite:version/pk', kwargs={"pk": 0}), content_type=APPLICATION)
    #
    #     self.assertEquals(response.status_code, 200)
    #     self.assertEquals(response.headers['Content-Type'], APPLICATION)
